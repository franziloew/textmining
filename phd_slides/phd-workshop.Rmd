---
title: "What is reported in the news?"
...
$$
\begin{aligned}
p(\beta,\theta,z|w) \propto
\displaystyle \prod_{i=1}^{K}p(\beta_i)\displaystyle \prod_{d=1}^{D}p(\theta_d)(\prod_{n=1}^Np(z_{d,n,}|\theta_d)p(w_{d,n}|\beta_{d,k},z_{d,n}))
\end{aligned}
$$
...
$$
p(\eta,z,\kappa,\gamma,\Sigma|w,X,Y)\propto (\displaystyle \prod^D_{d=1}(\eta_d|X_d,\gamma,\Sigma)( \prod^N_{n=1}(z_{n,d}|\theta_d)(w_n|\beta_{d,k=z_{d,n}}))) \prod p(\kappa)\prod p(\Gamma)
$$

* The number of possible topic structures is exponentially large
    -> sum is intractable to compute.
...
plot(stmOut, type = "summary", xlim = c(0,.18), n = 5, 
...
3. **Relationship between topics** How topics are correlated differently for different newswires, indicating how topics are connected and framed differently in each newswire.
