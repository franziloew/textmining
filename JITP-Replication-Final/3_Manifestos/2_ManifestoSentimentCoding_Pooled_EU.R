###################################################
# Scoring MANIFESTO quasi-sentences of the 
# pro-EU (per108) and anti-EU (per110) categories
# and establish standard net-EU support measure
#
...
# Load and prepare the Manifesto sample
#######################################

load("./3_Manifestos/Data/AT_DE_CH_Sample.Rdata") 
data <- de.manifesto

# Keep only observations on the two EU items
de.manifesto <- de.manifesto[de.manifesto$cmp_code == 108 | de.manifesto$cmp_code == 110, ]
de.manifesto <- de.manifesto[order(de.manifesto$party, de.manifesto$date), ]


# Pooling categories by party, election and issue category (not direction!)
#---------------------------------------------------------------------------

# Aggregate the numerical variables

help1 <- aggregate(de.manifesto, by = list(de.manifesto$manifesto_id, de.manifesto$issue), FUN = mean)
help1$manifesto_id <- help1$Group.1
help1$issue <- help1$Group.2
help1 <- help1[ , c("manifesto_id", "party", "date", "issue")]

# Aggregate (paste) the manifesto direction codings
# Order should be the same

help2 <- aggregate(de.manifesto$direction, by = list(de.manifesto$manifesto_id, de.manifesto$issue), FUN = paste, collapse = " ")


# Aggregate (paste) the manifesto text per category 
# Order should be the same

help3 <- aggregate(de.manifesto$text, by = list(de.manifesto$manifesto_id, de.manifesto$issue), FUN = paste, collapse = " ")

# Combine

help1$direction <- help2$x
help1$text <- help3$x

# Get aggregate direction of the human codes

help1$pos <- str_count(help1$direction, "positive")
help1$neg <- str_count(help1$direction, "negative")

# Establish the standard EU net support measure

help1$agg.direction <- (help1$pos - help1$neg) / (help1$pos + help1$neg) 
hist(help1$agg.direction, breaks =35)

# The final data set to be used for scoring and comparisons
...
data$sent.text2  <- data$sent.text # Copy of the sampled sentences
...
save(data, file = "./3_Manifestos/Data/ValidationSamplePooledScored.Rdata")
