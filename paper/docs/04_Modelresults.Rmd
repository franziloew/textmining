...
topics <- matrix(c(1, "SPD, M.Schulz", 2, "A.Merkel vs. Schulz", 3, "B90/Die Grüne, coalition talks", 
                   ...
                   29, "AfD in parliament", 30, "Mix: Sexism, Court sentences, NSU trial", 
                   31, "German armed forces, v.d.Leyen", 
                   ...
                   36, "Church, Family", 37, "Car industry, Stuttgart (Mix)", 38, "AfD, F.Petry", 
                   39, "Statistics, welfare policy",
...
sentDF.values <- sentDF %>%
  ...
  left_join(., btw.2 %>% filter(topic %in% keep), 
            by = "document") %>%
  # calculate sentiment
  mutate(sentiment = sent_diff / text_length)
```

#### 4. Inspect sentiment value

```{r fig.width=8, fig.height=12}
p <- sentDF.values %>%
  group_by(topic_name, date, site) %>%
  summarise(sentiment = mean(sentiment, na.rm=T))

p %>%
  filter(grepl("Merkel", topic_name)) %>%
