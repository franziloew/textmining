...
ggsave(filename = "../figs/topic_proportion.png", height = 10)
```
...
ggsave(plot = p1, filename = "../figs/estimates.png", device = "png",width = 8, height = 5,
dpi = 600)
```
...
#   filter(mean_val != "NaN") %>%
#   mutate(mean_val = normalize_data(mean_val)) %>%
  ...
  for (corr1Id in 1:ncol(corr2)) {
    ...
    for(corr2Id in 1:ncol(corr1)) {
      ...
      ggplot(corr, aes(Var2, Var1,
        ...
        labs(x="", y="", fill="",
             title = "Cross-Correlation / Polls vs. Sentiment Value",
             ...)
}
```
...
```{r}
plot_Poll_Sent <- function(filters, party_str, color_str) {
  
  ### Plot 1  
  plot1 <- group1 %>%
    filter(grepl(filters, topic_name))
...
##### 3.2.1. CDU
```{r fig.height=12, fig.width=10}
filters <- paste("CDU","CSU","Great Coalition", sep="|")

### Plot 1  
plot_Poll_Sent(filters, "Union", "black")
```


#### SPD
```{r fig.height=12, fig.width=10}
filters <- paste("SPD","Great Coalition", sep="|")

plot_Poll_Sent(filters, "SPD", "black")
```

##### 3.2.3. AfD
```{r fig.height=12, fig.width=10}
filters <- paste("AfD", sep="|")

plot_Poll_Sent(filters, "AfD", "black")
```

#### FDP
```{r fig.height=12, fig.width=10}
filters <- paste("Jamaica", sep="|")

plot_Poll_Sent(filters, "FDP", "black")
```

#### Grüne
```{r fig.height=12, fig.width=10}
filters <- paste("Grüne","Jamaica", sep="|")

plot_Poll_Sent(filters, "Grüne", "black")
```