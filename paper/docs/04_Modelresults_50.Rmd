...
btw.2 %>% 
  mutate(yearweek = calculate_week(week(date), year(date))) %>%
  group_by(yearweek) %>%
  summarise(total_obs = n()) -> tally_week
...
btw.2 %>% 
  ...
  summarise(total_obs = n()) -> tally_week_site
```
