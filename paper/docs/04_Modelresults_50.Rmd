...
polls.red <- polls %>%
  ...
  mutate(Union = normalize_data(Union))
...
### Plot 1  
p1 <- ggplot() +
  geom_col(data=p, aes(yearmonth, sentiment_n, 
             group = topic_name,
             fill = topic_name),
           position = "dodge", 
           alpha = 0.8) +
...
polls.red <- polls %>%
  filter(yearmonth > as.Date("2017-05-01")) %>%
  mutate(FDP = normalize_data(FDP))

topics.red <- sentDF.values %>%
  filter(grepl(paste("Jamaica", sep="|"), 
               topic_name, ignore.case = T))
...
polls.red <- polls %>%
  filter(yearmonth > as.Date("2017-05-01")) %>%
  mutate(Grüne = normalize_data(Grüne))

topics.red <- sentDF.values %>%
  filter(grepl(paste("Jamaica","Grüne", sep="|"), 
               topic_name, ignore.case = T))
