...
ggsave(plot = {
  btw %>%
    ggplot(aes(as.numeric(text_length.cut),
                       group = site)) +
    geom_histogram(alpha=.8, fill = col[3]) + 
...
dev.off()
```
![Wordcloud 1](../figs/wordcloud.png)
...
wordclouds <- function(select_site) {
  corp <- btw %>% filter(site %in% select_site)
  corp <- corpus(corp$text_cleaned1)
  corp <- dfm(corp)

  textplot_wordcloud(corp, max.word=200)
}
```
...
<img src="../figs/wordcloud_Bild.de.png", width = "49%" title="Bild.de">
<img src="../figs/wordcloud_DIE WELT.png", width = "49%" title="DIE WELT">
<img src="../figs/wordcloud_FOCUS ONLINE.png", width = "49%" title="FOCUS ONLINE">
<img src="../figs/wordcloud_SPIEGEL ONLINE.png", width = "49%" title="SPIEGEL ONLINE">
<img src="../figs/wordcloud_stern.de.png", width = "49%" title="Stern.de">
<img src="../figs/wordcloud_Tagesschau.de.png", width = "49%" title="Tagesschau.de">
<img src="../figs/wordcloud_ZEIT ONLINE.png", width = "49%" title="ZEIT ONLINE">
...
print(xtable(coeff_tab[1:nr,], type="latex"), file="../writing/tables/coeff1.tex" )
print(xtable(coeff_tab[nr:(nr*2),], type="latex"), file="../writing/tables/coeff2.tex" )
print(xtable(coeff_tab[(nr*2):(nr*3),], type="latex"), file="../writing/tables/coeff3.tex" )
print(xtable(coeff_tab[(nr*3):(nr*4),], type="latex"), file="../writing/tables/coeff4.tex" )
print(xtable(coeff_tab[(nr*4):(nr*5),], type="latex"), file="../writing/tables/coeff5.tex" )
```
...
p1 <- ggplot(p, 
 ...
 geom_line(color = col[5], size = 0.6) +
  ...
  geom_hline(yintercept = 0, linetype = 2,
             color = "black") +
  labs(x="", y="Sentiment Score (weighted)",
        title = "",
       subtitle = "") +
...
sentDF.values %>%
  mutate(topic_name = ifelse(grepl("B90", topic_name),
                             "B90/\nDie Grünen", topic_name),
         topic_name = ifelse(grepl("17", topic_name),
                             "17: Debates\nwithin SPD", topic_name),
         topic_name = ifelse(grepl("13", topic_name),
                             "13: A.Merkel,\nelection campaign", topic_name),
         topic_name = ifelse(grepl("30", topic_name),
                             "30: AfD,\nF.Petry & Meuthen", topic_name),
         topic_name = ifelse(grepl("32", topic_name),
                             "32: AfD,\nGauland & Weidel", topic_name),
         topic_name = ifelse(grepl("37", topic_name),
                             "37: AfD & DIE LINKE\nin parliament", topic_name),
         topic_name = ifelse(grepl("20", topic_name),
                             "20: AfD, right-wing\nradicalism", topic_name)
         ) %>%
...
p <- radar %>%
  ggRadar(aes(color=site), 
          rescale = F,
          alpha = 0) +
  labs(title = "Sentiment scores Jun 2017-March 2018",
       subtitle = "weighted") +
...
ggsave(filename = "../figs/sentscore_radar.png", 
       plot = p)
```
...
The variance of the cross-correlation coefficient under the null hypothesis of zero correlation is approx $1/n$ where $n$ is the length of the series. The coefficients are also asymptotically normal. So approximate critical values (at the 5% level) are $±2/\sqrt{n}$.
