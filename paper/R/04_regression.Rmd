...
load("../output/models/STM 28 2.Rda")
```
...
ggsave(
  plot = {
    posts_classification.sdt %>% 
      ggplot(aes(x = 0, y = row, color = factor(topic, levels = 1:28))) +
      facet_wrap(~topic_title, ncol = 4) +
      geom_text(
        aes(label = title),
        size = 3, hjust = 0, family = "Roboto"
      ) +
...
```{r}
prep <- estimateEffect(1:28 ~site + s(month),
