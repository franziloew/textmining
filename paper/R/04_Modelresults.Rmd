...
topics <- matrix(c(1, "Great coalition", 2, "A.Merkel, CSU, Guttenberg", 3, "Jamaica coalition",
                   ...
                   7, "political trends in GER", 8, "Merkel vs.Schulz",
                   9, "Trends in electoral districts", 10, "Steinmeier, Federal President",
                   11, "G.Schröder & IT themes (Data policy, Wifi, Telekom)",
                   12, "Federal Election trends", 13, "Climate policy, Die Grünen", 14, "G20 Hamburg",
                   15, "error (text processing failure)", 16, "Public authorities (police, BKA)",
                   17, "Federal budget statistics", 18, "Family reunion refugees",
                   19, "Election Polls", 20, "AfD in social media", 21, "(Sexual) assaults, Gender Debate, M.Schwesig", 
                   22, "elections in Niedersachsen", 23, "Court judgements (on terror attacks, NSU)", 
                   24, "election campaigns & political talkshows", 25, "German armed forces",
                   26, "Issues in the Bundestag", 27, "C.Lindner FDP", 28, "Diplomatic relations w Turkey & Russia", 
                   29, "A.Merkel in east GER", 30, "H.Kohl obituary", 31, "AfD (Petri, Gauland, Höcke)", 
                   32, "Refugees (in GER)", 33, "EU policies", 34, "DIE LINKE", 35, "SPD", 
                   36, "Islam and antisemitism in GER", 37, "AfD in parliament", 38, "Church", 
                   39, "Terror attack in Berlin (Amri)", 40, "DHL package attack (Potsdam)"), ncol=2, byrow=T)

# topics <- matrix(c(1, "Great coalition", 2, "A.Merkel, CSU, Guttenberg", 3, "Coalition talks (Jamaica, Niedersachsen)",
#                    4, "Diesel scandal", 5, "A.Merkel, glyphosat scandal", 6, "DIE LINKE", 
#                    7, "Islam and antisemitism in GER", 8, "Merkel vs.Schulz", 
#                    9, "Trends in electoral districts", 10, "Steinmeier, Federal President", 
#                    11, "G.Schröder & IT themes (Data policy, Wifi, Telekom)", 
#                    12, "Federal Election trends", 13, "EU policies", 14, "G20 Hamburg", 
#                    15, "error (text processing failure)", 16, "Refugees in GER", 
#                    17, "Budget statistics, social welfare", 18, "Jamaica Coalition talks", 
#                    19, "Polls federal election", 20, "AfD in social media", 
#                    21, "Women empowerment, M.Schwesig (SPD)", 22, "AfD (Petri, Gauland, Höcke)", 
#                    23, "Court judgements on terrorist attacks", 24, "election campaigns & political talkshows", 
#                    25, "German armed forces", 26, "Issues in the Bundestag", 27, "C.Lindner FDP", 
#                    28, "Turkey, Russia", 29, "Terror attacks (GER)  & police reports", 30, "H.Kohl obituary"), ncol=2, byrow=T)
...
```{r}
btw.2 %>% filter(topic==16) %>% select(title, gamma, site) %>%
  arrange(desc(gamma)) %>%
...
freq1 <- freq %>%
  ...
  left_join(., frequency %>% mutate(topic_name = topic) %>%
              select(-topic),
            by = "topic_name")

p2 <- ggplot(freq1, aes(reorder(topic_name,frequency), freq_diff,
                 fill = factor(value))) +
...
```{r fig.height=6}
plotQuote(c(paste(sagelabs$cov.betas[[1]]$problabels[7,], collapse="\n"),
            paste(sagelabs$cov.betas[[2]]$problabels[7,], collapse="\n")))
```
...
```{r}
effect <- estimateEffect(c(1:k) ~ownership+s(month), stmOut, 
...
## 3.5 Plotquote
Take a closer look to those topics that are discussed most different. 

```{r fig.height=6}
plotQuote(c(paste(sagelabs$cov.betas[[1]]$problabels[23,], collapse="\n"),
            paste(sagelabs$cov.betas[[2]]$problabels[23,], collapse="\n")))
```