...
```{r}
sagelabs$cov.betas[[1]]$problabels[,1:6]
```
...
y <- length(stmOut$settings$covariates$yvarlevels)
```
...
for (i in 1:y) {
  ...
  for (j in 1:k) {
    ...
    #print(paste("topic = ", j))
    kld.matrix[x,j] <- KLD(base[j,],compare1[j,])
    kld.matrix[x+1,j] <- KLD(base[j,],compare2[j,])
...
for (i in 1:y) {
  ...
  for (j in 1:k) {
    ...
    #print(paste("topic = ", j))
    jsd.matrix[x,j] <- JSD(base[j,],compare1[j,])
    jsd.matrix[x+1,j] <- JSD(base[j,],compare2[j,])
    # kld.matrix[x+2,j] <- JSD(base[j,],compare3[j,])
    # kld.matrix[x+3,j] <- JSD(base[j,],compare4[j,])
    # kld.matrix[x+4,j] <- JSD(base[j,],compare5[j,])
    # kld.matrix[x+5,j] <- JSD(base[j,],compare6[j,])
...
ggsave(filename = "../figs/distance.png", device = "png",width = 10, height = 8,
dpi = 600)
```
