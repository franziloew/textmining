...
# 3. Model Results
```{r caching, echo = FALSE}
load("../output/models/STM_ownership 30 .Rda")
...
topics <- matrix(c(1, "Great coalition", 2, "A.Merkel, CSU, Guttenberg", 3, "Coalition talks (Jamaica, Niedersachsen)", 
                   ...
                   15, "error (text processing failure)", 16, "Refugees, asylum claims", 
                   ...
                   23, "Terror attack Berlin (Amri)", 24, "election campaigns & political talkshows", 
...
#                    7, "political trends after the election", 8, "Merkel vs.Schulz", 
#                    9, "Trends in electoral districts", 10, "Steinmeier, Federal President", 
#                    11, "G.Schröder & IT themes (Data policy, Wifi, Telekom)", 
#                    12, "Federal Election trends", 13, "Climate policy, Die Grünen", 14, "G20 Hamburg", 
#                    15, "error (text processing failure)", 16, "Police academy (Berlin), AfD", 
#                    17, "Federal budget statistics", 18, "Family reunion refugees", 
#                    19, "Polls federal election", 20, "AfD in social media", 
#                    21, "Women empowerment, M.Schwesig (SPD)", 22, "elections in Niedersachsen", 
#                    23, "Court sentences, NSU trial", 24, "election campaigns & political talkshows", 
#                    25, "German armed forces", 26, "Issues in the Bundestag", 27, "C.Lindner FDP", 
#                    28, "Turkey, Russia", 29, "A.Merkel in east GER", 30, "H.Kohl obituary", 
#                    31, "AfD (Petri, Gauland, Höcke)", 32, "Asylum claims and deportation", 
#                    33, "EU policies", 34, "DIE LINKE", 35, "SPD", 36, "Islam and antisemitism in GER", 
#                    37, "AfD in parliament", 38, "Church", 39, "Terror attacks (GER)  & police reports", 
#                    40, "DHL package attack (Potsdam)"), ncol=2, byrow=T)
...
```{r}
btw.2 %>% filter(topic==17) %>% select(title, gamma, site) %>%
  arrange(desc(gamma)) %>%
...
```{r}
p2 <- ggplot(freq, aes(reorder(topic_name,frequency), freq_diff,
                 fill = factor(value))) +
...
# ggsave(plot = p1, filename = "../figs/estimates_site.png", device = "png",width = 9, height = 7,
# dpi = 600)
```
...
ggsave(plot = p1, filename = "../figs/estimates_month.png", device = "png",width = 9, height = 7,
dpi = 600)
```
...
```{r}
plotQuote(c(paste(sagelabs$cov.betas[[1]]$problabels[27,], collapse="\n"),
            paste(sagelabs$cov.betas[[2]]$problabels[27,], collapse="\n")))
```

```{r}
btw.2 %>%
  filter(grepl("sturm", url) &
           site == "Tagesschau.de") %>%
  select(title, site)
```
