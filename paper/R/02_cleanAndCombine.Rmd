...
```{r}
public <- c("deutschlandfunk.de", "heute.de", "ndr.de", "swr.de", "tagesschau.de", "wdr.de", "zdf.de")

df_full <- df_full %>% 
  mutate(ownership = ifelse(site %in% public, "public", "private"),
         month = month(date))

df_full %>% 
  #filter may2017 and january2018
  filter(month>5) %>%
  #calculate site totals by month
  group_by(site, month) %>%
  summarize(site_sum = n()) %>%
  ungroup() %>%
  #calculate month total
  group_by(month) %>%
  mutate(month_sum = sum(site_sum)) %>%
  ungroup() %>%
  #calculate share
  mutate(share = 100*(site_sum/month_sum))-> totals_site

df_full %>% 
  #filter may2017 and january2018
  filter(month>5) %>%
  # calculate ownership totals by month
  mutate(ownership = ifelse(site %in% public, "public", "private")) %>%
  group_by(ownership, month) %>%
  summarize(owner_sum = n()) %>%
  #calculate month total
  group_by(month) %>%
  mutate(month_sum = sum(owner_sum)) %>%
  ungroup() %>%
  #calculate share
  mutate(share = 100*(owner_sum/month_sum))-> totals_ownership
```
...
#df %>% filter(site=="tagesschau.de") %>% select(url)
```
...
# Process data
processed1 <- textProcessor(btw$text_cleaned, metadata = btw[,c("site","text_cleaned","month")],
                           wordLengths = c(2,Inf),
                           lowercase = F,
                           removestopwords = F,
                           removenumbers = F,
...
processed2 <- textProcessor(btw$text_cleaned1, metadata = btw[,c("site","text_cleaned1","month")],
                           wordLengths = c(2,Inf),
                           lowercase = F,
                           removestopwords = F,
                           removenumbers = F,
                           removepunctuation = F,
                           stem = F)
out_stemmed <- prepDocuments(processed2$documents, processed2$vocab, processed2$meta)

save(btw, out1, out_stemmed, file="../output/btw_combined.Rda")
```
