...
okt <- read_csv("../data/eventregistry/eventbride_17_10_18.csv")
nov_03 <- read_csv("../data/eventregistry/eventbride_17_11_03.csv")
nov_22 <- read_csv("../data/eventregistry/eventbride_17_11_22.csv")
nov_30 <- read_csv("../data/eventregistry/eventbride_17_11_30.csv")
dec_16 <- read_csv("../data/eventregistry/eventbride_17_12_16.csv")
jan_1 <- read_csv("../data/eventregistry/eventbride_18_01_1.csv")
```

## combine dataframes

```{r}
df_full <- bind_rows(list(okt,nov_03,nov_22,nov_30,dec_16,jan_1)) %>%
  mutate(text = body,
         # Extract site 
...
```{r}
keeps <- c("bild.de","focus.de", "zeit.de","spiegel.de", "stern.de",
...
df %>%
  mutate(politics = ifelse(grepl(pat, title_text, ignore.case = TRUE),1,0)) -> df
```
...
### without stemming
processed <- textProcessor(btw$text_cleaned1, metadata = btw[,c("ownership","text_cleaned1","month")],
                           wordLengths = c(2,Inf),
                           lowercase = F,
                           removestopwords = F,
                           removenumbers = F,
...
out$meta$ownership <- as.factor(out$meta$ownership)
###
...
save(btw, out, file="../output/btw_comb.Rda")
```
