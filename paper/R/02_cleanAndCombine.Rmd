...
nov_30 <- read_csv("../data/eventbride/eventbride_nov_30.csv")
```

## combine dataframes

```{r}
btw1 <- bind_rows(list(okt,nov_03,nov_22,nov_30)) %>%
  mutate(text = body,
         # Extract site 
...
btw %>%
  mutate(politics = ifelse(grepl(paste(pat1,pat2,pat3,pat4,pat5,
                     sep="|"), url, perl = TRUE),1,0)) -> df_full
```

```{r}
df_full %>%
  filter(politics == 1) %>%
  distinct(url, .keep_all = T) -> btw
```
...
```{r}
btw %>%
  filter(grepl("faktenfinder", title_text)) %>%
