...
# ggsave("../figs/timeline.png", plot = last_plot(), device = "png", 
#        dpi = 600)
```
...
# ggsave("../figs/bar.png", plot = last_plot(), device = "png", 
#        dpi = 600)
```

```{r message=FALSE, warning=FALSE}
btw %>%
  ggplot(aes(date, site, size=text_length, color = site)) +
  geom_point(show.legend = FALSE, alpha = .8) +
  scale_color_manual(values = col) +
  labs(x="", y="") 

# ggsave("../figs/fb_shares.png", plot = last_plot(), device = "png", 
#        dpi = 600)
```

```{r message=FALSE, warning=FALSE}
btw %>%
  ggplot(aes(site, text_length)) +
  stat_summary(fun.y="median", geom="bar", fill = col, alpha = .8) +
  labs(x="", y="") 

#ggsave("../figs/fb_shares_mean.png", plot = last_plot(), 
#       dpi = 600)
```
...
t1 <- Sys.time()
kresult <- searchK(out$documents, out$vocab, K, prevalence=~site + s(btw_dummy), data = out$meta, init.type = "Spectral")
t2 <- Sys.time()
t2 - t1
...
We then train the Structural Topic Model on the full dataset, with $k = 50$.
...
#### Topic Summary
```{r}
plot(stmOut, type = "summary", xlim = c(0,.14), n = 5, #labeltype = "frex",
         main = "Online News Topics", text.cex = 0.8)
```

The most frequent words in the model such as for topics #3, #7, and #20
```{r}
plot(stmOut, type="labels", topics=c(37,8))
```

A comparison of two topics 
```{r}
plot(stmOut, type="perspectives", topics=c(37,8))
```
...
prep <- estimateEffect(1:50 ~ site+s(month), stmOut, meta=out$meta, uncertainty = "Global")
```

To see how prevalence of topics differs across values of a continuous covariate:
```{r}
plot(prep, "month", method="continuous", topics = c(46,42),
     xlab="Month (2017)")
monthseq <- seq(from=as.Date("2017-06-01"), to=as.Date("2017-11-01"), by="month")
monthnames <- month(monthseq)
axis(1, at=as.numeric(monthseq), labels = monthnames)
```
...
To get an idea of the topics, we take a look at the words that are associated with topics and label them accordingly. We then inspect the documents that are estimated to be highly associated with each topic. Figure --- shows a random sample of 7 posts from the 300 highest probability fits per topic.
...
topicLabel <- as.data.frame(label$prob)
topicLabel <- topicLabel %>%
  transmute(topic = rownames(.),
    topic_name = paste(V1,V2,V3,V4, sep = ", "))
            
frex <- as.data.frame(label$frex)
topicLabel <- topicLabel %>%
  left_join(., 
            frex %>%
              transmute(topic = rownames(.),
                frex = paste(V1,V2,V3,V4, sep=", ")),
            by = "topic")

lift <- as.data.frame(label$lift)
topicLabel <- topicLabel %>%
  left_join(., 
            lift %>%
              transmute(topic = rownames(.),
                lift = paste(V1,V2,V3,V4, sep=", ")),
            by = "topic")

score <- as.data.frame(label$score)
topicLabel <- topicLabel %>%
  left_join(., 
            score %>%
              transmute(topic = rownames(.),
                score = paste(V1,V2,V3,V4, sep=", ")),
            by = "topic")
...
![Facebook News Map](../figs/news-landscape-map.png)
...
```{r Facebook Shares}
btw2 %>%
  ggplot(aes(as.factor(topic), fb_shares)) +
  stat_summary(fun.y="sum", geom="bar", fill = "blue", alpha=.7) +
  labs(x="", y="Sum") +
  scale_x_discrete(breaks=c(1:50), labels = topicLabel$topic_name) +
  theme(axis.text.x = element_text(size=7, angle=90),
        axis.text.y = element_text(size=7))

ggsave("../figs/fb_shares_topics.png", device = "png", 
       dpi = 600)
```
...
btw.reg %>%
  mutate(topic.f = factor(topic),
         site.f = factor(site)) -> btw.reg
```

```{r}
btw.reg %>%
  filter(fb_shares < 500) %>%
  ggplot(aes(fb_shares, fill = site)) +
  geom_histogram(binwidth = 10)
```


```{r}
linreg <- lm(fb_shares ~ topic.f*site, data=btw.reg)
...
posts_classification.dt %>% 
  inner_join(
    btw2 %>% mutate(document = as.numeric(articleID)) %>%
