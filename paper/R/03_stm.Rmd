...
```{r}
btw %>%
  ggplot(aes(date, site, size=fb_shares, color = site)) +
...
```{r}
btw %>%
  ggplot(aes(site, fb_shares)) +
  stat_summary(fun.y="mean", geom="bar", fill = col, alpha = .8) +
  labs(x="", y="") 
...
# Process data
processed <- textProcessor(btw$text_cleaned, metadata = btw[,c("site","text_cleaned")],
                           wordLengths = c(2,Inf),
                           lowercase = F,
                           removestopwords = F,
                           removenumbers = F,
...
We then train the Structural Topic Model on the full dataset, with $k = 40$.
...
btw %>%
  ...
  left_join(., topicLabel %>%
              ...
              mutate(topic = as.numeric(topic)), by="topic") -> btw2

# subset  
topics_mapping.dt <- btw2 %>%
  group_by(topic) %>%
  slice(1) %>%
  select(topic, title.text, articleID, topic_name) 
```
...
posts_classification.sdt <-
  ...
  ungroup() %>% 
  inner_join(btw2 %>% select(title, articleID) %>%
               mutate(articleID = as.numeric(articleID)), by = "articleID") %>% 
...
btw %>%
  ...
  labs(x="", y="Sum") +
  scale_x_discrete(breaks=c(1:40), labels = topicLabel$topic_name) +
