...
doctopics.df %>%
  transmute(topic = V1) -> doctopics.df
doctopics.df$articleID <- as.integer(rownames(doctopics.df))

# Add Topic to origian DF
btw %>%
  inner_join(.,doctopics.df, by="articleID") -> btw

topics_mapping.dt <- btw %>%  
  group_by(topic) %>%
  slice(1) %>%
  select(topic,title, title.text, articleID) 
```
...
posts.dtp %>% 
  group_by(articleID = as.numeric(document)) %>% 
  summarise(topic = min(topic[gamma == max(gamma)]), gamma = max(gamma)) %>% 
  ungroup()  %>%
  inner_join(btw %>% select(title, articleID), by = "articleID") %>%
...
ggsave(
  plot = {
    btw %>%
      ggplot(aes(as.factor(topic), fill =site)) +
      geom_bar() +
      scale_x_discrete(labels = paste0(topicLabel$topic,":",topicLabel$topic_name)) +
      theme(axis.text.x = element_text(size=8, angle=90),
        legend.text = element_text(size = 8)) +
      labs(x = "", fill = "", 
           title    = "Topic Frequency by Media",
        subtitle = "") 

  },
  
  filename = "../figs/01-topic-distribution.png",
  device = "png",
  dpi = 600
    )
```
![Facebook news samples](../figs/01-topic-distribution.png)
...
posts_classification.dt %>% 
  ...
  inner_join(
    btw %>% 
      select(document = articleID, date, 
             title, articleID) %>% 
      mutate(post_period = 
               date %>% 
  ...
  left_join(topics_mapping.dt %>% select(topic, topic_name), by = "topic") %>%
  # Summarise into tidy dataframe
  group_by(post_period, topic_name) %>% 
  summarise(
    articles = sum(allocation),
    sample_articles = 
      paste0("<li>", sample(title, 3, replace = TRUE ), "</li>", collapse = "<br>")
  ) %>% 
  ungroup() %>%
  mutate(
    tooltip_text = 
...
posts_classification.dt %>% 
  inner_join(
    btw %>% select(document = articleID, news_page = site, title),
    by = "document"
  ) %>%
  inner_join(topics_mapping.dt, by = "topic") %>% 
