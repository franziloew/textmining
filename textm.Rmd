...
- At an abstract level, text is simply a string of characters 
    - Latin alphabet ('a', 'A', 'p' and so on), Punctuation (e.g. !), Numbers,Non-alphanumerical characters (e.g. @)
...
1. **Pre-process** the text (Stopword removal, tokenization, stemming) and represent document $D$ as a numerical array $\boldsymbol{C}$ (e.g. bag-of-words model)
...
htmlTable(orig[c(1,4,5,7),c("text","text_cleaned")], align = "l")
```

## Tokenization

```{r, echo=FALSE, message=FALSE, warning=FALSE}
orig$text_cleaned <- removeWords(orig$text_cleaned, c("cdu","csu","union","spd","fdp","grüne","grünen","linke","afd","btw"))

orig %>%
  #group_by(partei) %>%
  unnest_tokens(word, text_cleaned) %>%
  count(word, sort = TRUE) -> tokens

orig %>%
  #group_by(partei) %>%
  unnest_tokens(bigram, text_cleaned, token="ngrams", n=2) %>%
  count(bigram, sort = TRUE) -> bigrams
```
...
- **IDF** Inverse Document Frequency $idf_v = log(\frac{D}{df_v})$, where $df_v$ is the number of documents that contain term $v$ and $D$ ist die number of Documents
...
tokens %>%
  top_n(5) %>%
  ggplot() +
  aes(x= reorder(word, tf_idf), y=tf_idf, fill = partei) +
...
tokens %>%
  top_n(5) %>%
  ggplot() +
  aes(x= reorder(word, tf), y=tf, fill = partei) +
  geom_col(alpha=.8, show.legend = FALSE) +
  labs(x = NULL, y = "tf") +
  scale_fill_manual(name="", values=col) +
  coord_flip() -> p2
...
**2. Latent attributes** (unsupervised methods): 
  ...
  - **Starting Point** The true value of $v_i$ is unobserved -> The function relating $\boldsymbol{c}_i$ to $\boldsymbol{v}_i$ is unknown
  
  - **Idea** Uncover hidden patterns that structure the observations in some meaningful way.
  
  - **Use** The outputs of algorithms can be used as inputs into econometric models for predicting some variable of interest.
...
Very popular topic model is the **Latent Dirichlet allocation**
